...
import com.revplay.analyticsservice.service.ArtistAnalyticsService;
...
import com.revplay.analyticsservice.util.AnalyticsValidationUtil;
...
public class ArtistAnalyticsServiceImpl implements ArtistAnalyticsService {
    ...
    private static final String ARTIST_EXISTS_SQL = "SELECT COUNT(1) FROM revplay_catalog.artists WHERE artist_id = ?";
    private static final String DASHBOARD_SQL = """
            SELECT
              ? AS artist_id,
              (SELECT COUNT(*) FROM revplay_catalog.songs s WHERE s.artist_id = ? AND s.is_active = true) AS total_songs,
              (SELECT COUNT(*) FROM revplay_playback.play_history ph
               JOIN revplay_catalog.songs s ON s.song_id = ph.song_id
               WHERE s.artist_id = ? AND s.is_active = true) AS total_plays,
              (SELECT COUNT(*) FROM revplay_playlists.user_likes ul
               JOIN revplay_catalog.songs s ON s.song_id = ul.likeable_id
    ...
    private static final String SONG_PLAY_COUNT_SQL = """
            ...
            FROM revplay_catalog.songs s
            LEFT JOIN revplay_playback.play_history ph ON ph.song_id = s.song_id
    ...
    private static final String SONG_POPULARITY_SQL = """
            ...
            FROM revplay_catalog.songs s
            LEFT JOIN revplay_playback.play_history ph ON ph.song_id = s.song_id
            LEFT JOIN revplay_playlists.user_likes ul ON ul.likeable_id = s.song_id AND ul.likeable_type = 'SONG'
    ...
    private static final String FAVORITED_USERS_SQL = """
            ...
            FROM revplay_playlists.user_likes ul
            JOIN revplay_catalog.songs s ON s.song_id = ul.likeable_id
            JOIN revplay_users.users u ON u.user_id = ul.user_id
            LEFT JOIN revplay_users.user_profiles up ON up.user_id = u.user_id
    ...
    private static final String TOP_LISTENERS_SQL = """
            ...
            FROM revplay_playback.play_history ph
            JOIN revplay_catalog.songs s ON s.song_id = ph.song_id
            JOIN revplay_users.users u ON u.user_id = ph.user_id
            LEFT JOIN revplay_users.user_profiles up ON up.user_id = u.user_id
            WHERE s.artist_id = ? AND s.is_active = true
            GROUP BY u.user_id, u.username, u.email, up.full_name
    ...
    public SongPlayCountResponse songPlayCount(Long artistId, Long songId) {
        ...
        return jdbcTemplate.query(
                ...
                () -> new ResourceNotFoundException("Song " + songId + " does not belong to artist " + artistId)
        );
    }
    ...
    public List<ListeningTrendPointResponse> listeningTrends(Long artistId, TrendRange range, LocalDate from, LocalDate to) {
        ...
        requireArtist(artistId);
        if (to.isBefore(from)) {
            throw new AnalyticsValidationException("'to' date must be greater than or equal to 'from' date");
        }
    ...
    public List<TopListenerResponse> topListeners(Long artistId, int limit) {
        ...
        requireArtist(artistId);
        AnalyticsValidationUtil.requireLimitInRange(limit, 1, 100);
    ...
    private void requireArtist(Long artistId) {
        if (artistId == null || artistId <= 0) {
            throw new AnalyticsValidationException("artistId must be a positive number");
        }
        ...
        if ((count == null ? 0L : count) == 0L) {
            ...
            throw new ResourceNotFoundException("Artist " + artistId + " does not exist");
        }
    }
