...
import com.revplay.analyticsservice.service.PlaybackAnalyticsService;
import com.revplay.analyticsservice.service.UserStatisticsService;
import com.revplay.analyticsservice.dto.response.ActiveUsersMetricsResponse;
import com.revplay.analyticsservice.dto.response.ContentPerformanceResponse;
import com.revplay.analyticsservice.dto.response.DashboardMetricsResponse;
import com.revplay.analyticsservice.dto.response.GenrePlayCountResponse;
...
public class PlaybackAnalyticsServiceImpl implements PlaybackAnalyticsService {
    ...
    private static final String TRENDING_PODCAST_SQL = """
            ...
            FROM revplay_playback.play_history ph
            JOIN revplay_catalog.podcast_episodes pe ON pe.episode_id = ph.episode_id
            WHERE ph.played_at >= ?
    ...
    private static final String TOP_ARTISTS_SQL = """
            ...
            FROM revplay_playback.play_history ph
            JOIN revplay_catalog.songs s ON s.song_id = ph.song_id
            JOIN revplay_catalog.artists a ON a.artist_id = s.artist_id
    ...
    private static final String TOP_GENRES_SQL = """
            ...
            FROM revplay_playback.play_history ph
            JOIN revplay_catalog.songs s ON s.song_id = ph.song_id
            JOIN revplay_catalog.song_genres sg ON sg.song_id = s.song_id
            JOIN revplay_catalog.genres g ON g.genre_id = sg.genre_id
            WHERE ph.user_id = ?
            GROUP BY g.name
            ORDER BY play_count DESC
            LIMIT 5
    ...
    private static final String PEAK_HOUR_SQL = """
            ...
            FROM revplay_playback.play_history
            WHERE user_id = ?
            GROUP BY HOUR(played_at)
            ORDER BY COUNT(*) DESC
            LIMIT 1
            """;
    private static final String TOTAL_PLAYS_SQL = "SELECT COUNT(*) FROM revplay_playback.play_history";
    private static final String PLAYS_LAST_24H_SQL = "SELECT COUNT(*) FROM revplay_playback.play_history WHERE played_at >= ?";
    private static final String ACTIVE_USERS_24H_SQL =
            "SELECT COUNT(DISTINCT user_id) FROM revplay_playback.play_history WHERE played_at >= ?";
    ...
    private List<TrendingContentResponse> queryTrendingByType(String normalized, Instant since, int limit) {
        ...
        throw new AnalyticsValidationException("type must be 'song' or 'podcast'");
    }
