...
import com.revplay.analyticsservice.mapper.UserStatisticsMapper;
import com.revplay.analyticsservice.repository.UserStatisticsRepository;
...
public class UserStatisticsServiceImpl implements UserStatisticsService {
    ...
    private static final String USER_EXISTS_SQL = "SELECT COUNT(1) FROM revplay_users.users WHERE user_id = ?";
    private static final String PLAYLISTS_SQL = "SELECT COUNT(*) FROM revplay_playlists.playlists WHERE user_id = ?";
    private static final String FAVORITES_SQL = "SELECT COUNT(*) FROM revplay_interaction.user_likes WHERE user_id = ?";
    private static final String LISTENING_TIME_SQL =
            "SELECT COALESCE(SUM(play_duration_seconds), 0) FROM revplay_playback.play_history WHERE user_id = ?";
    private static final String SONGS_PLAYED_SQL =
            "SELECT COUNT(*) FROM revplay_playback.play_history WHERE user_id = ? AND song_id IS NOT NULL";
