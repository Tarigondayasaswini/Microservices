...
public class SearchServiceImpl implements SearchService {
    ...
    public PagedResponseDto<SearchResultItemResponse> search(SearchRequest request) {
        ...
        log.info("Executing search type={}, query={}, page={}, size={}, offset={}", type, request.query(), page, size, offset);

        if (!hasRequiredTables(type)) {
            log.warn("Search skipped because required tables are missing for type={}", type);
        ...
        try {
            ...
            return switch (type) {
                ...
                default -> throw new BadRequestException("Unsupported search type");
            };
        } catch (DataAccessException ex) {
            log.error("Search query failed for type={}", type, ex);
            return emptyPage(page, size, sortBy, sortDir);
        }
    }
    ...
    private PagedResponseDto<SearchResultItemResponse> searchAcrossAllTypes(
        ...
        // Include playlists in global search
        try {
            org.springframework.data.domain.Page<com.revplay.catalogservice.integration.playlist.PlaylistResponse> playlistPage = 
                playlistSearchService.searchPlaylists(query, org.springframework.data.domain.PageRequest.of(0, size));
            playlistPage.getContent().forEach(p -> combined.add(new SearchResultItemResponse(
                "playlist",
                p.getId(),
                p.getName(),
                null,
            ...
            log.warn("Failed to include playlists in global search", e);
        }
    ...
    private String normalizeSortBy(String sortBy) {
        ...
        if (!normalized.equals("releaseDate") && !normalized.equals("title") && !normalized.equals("contentId")) {
            throw new BadRequestException("sortBy must be one of: releaseDate, title, contentId");
        }
        return normalized;
    }
    ...
    private void validateDateRange(LocalDate from, LocalDate to) {
        if (from != null && to != null && from.isAfter(to)) {
            throw new BadRequestException("releaseDateFrom must be before or equal to releaseDateTo");
        }
    }
    ...
    private PagedResponseDto<SearchResultItemResponse> emptyPage(int page, int size, String sortBy, String sortDir) {
        return PagedResponseDto.empty(page, size, sortBy, sortDir);
    }
}
