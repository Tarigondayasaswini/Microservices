...
import com.revplay.playbackservice.mapper.PlayHistoryMapper;
import com.revplay.playbackservice.repository.PlayHistoryRepository;
import com.revplay.playbackservice.service.PlayHistoryService;
...
public class PlayHistoryServiceImpl implements PlayHistoryService {
    ...
    private boolean hasActiveSongOrNoSong(PlayHistory history) {
        ...
        try {
            return catalogServiceClient.isSongActive(history.getSongId()).getBody().getData();
        } catch (Exception e) {
            LOGGER.warn("Failed to check song status for songId={}, assuming inactive", history.getSongId());
            return false;
        }
    }
